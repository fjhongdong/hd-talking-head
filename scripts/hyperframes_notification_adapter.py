"""Bind the native HyperFrames Notification Cascade to the HD executor."""
...
ENTRY = "scripts/render_hyperframes_notification.cjs"
TEMPLATE_ID = "hyperframes/notification-cascade"
VERSION = "1.0.0"
...
def create_adapter(*, runtime_root, node_executable, browser_executable,
    ...
    return ReferenceProcessAdapter(
        adapter_id="native-hyperframes-notification-v1", dependency_id="hyperframes",
...
def create_artifact_probe(*, ffprobe_executable):
    ...
    def probe(path, media_type, contract):
        if media_type != "video" or contract["alpha"]:
            raise ValueError("Notification Cascade probe accepts only opaque video")
    ...
    return ArtifactProbe(probe_id="native-hyperframes-notification-ffprobe",
